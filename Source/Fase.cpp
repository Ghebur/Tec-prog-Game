...
namespace Fases {
    ...
    void Fase::adicionarEntidade(Entidades::Entidades* entidade) {
        if (entidade != nullptr) {
            entidades.incluir(entidade);
        }
    }

    void Fase::limparParaLoad() {
    // 1. Limpa as memórias do Gerenciador de Colisões (esquecendo os fantasmas)
    gerenciador.limpar();

    // 2. Percorre a lista e deleta as entidades físicas da memória
    auto* atual = entidades.getPrimeiro();
    while (atual != nullptr) {
        Entidades::Entidades* ent = atual->getInfo();
        if (ent != nullptr) {
            delete ent; 
        }
    ...
    // 3. Usa o SEU método da Lista para deletar os "Elementos" vazios
    entidades.limpar(); 
}
    void Fase::salvarFase() {
        ...
        while (atual != nullptr) {
            ...
            if (entidade != nullptr) {
                ...
                // Tenta converter a entidade para um Inimigo
                Personagens::Inimigo* ini = dynamic_cast<Personagens::Inimigo*>(entidade);
                
                // Se a conversão deu certo (É UM INIMIGO) e ele NÃO ESTÁ VIVO, pula pro próximo!
                if (ini != nullptr && !ini->estaVivo()) {
                    atual = atual->getProximo();
                    continue; 
                }

                // Se for obstáculo, plataforma ou um inimigo VIVO, salva normal!
                entidade->salvar();
            }
            atual = atual->getProximo();
        }
